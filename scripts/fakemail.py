...
# support EHLO
smtpd.SMTPChannel.smtp_EHLO = smtpd.SMTPChannel.smtp_HELO
...
class FakeServer(smtpd.SMTPServer):
    ...
    def __init__(self, localaddr, remoteaddr, path, open_mail=False):
    ...
    def process_message(self, peer, mailfrom, rcpttos, data):
        ...
        for recipient in rcpttos:
            ...
            f.write(data + "\n")
            f.close()
            message("Mail to %s saved to %s" % (recipient, filename))
            if self.open_mail:
                os.system('open -a Microsoft\ Outlook %s' % filename)
...
def read_command_line():
    ...
    for opt, arg in optlist:
        if opt == "--host":
            ...
            open_mail = True
    return host, port, path, background, open_mail
...
def main():
    global progname
    handle_signals()
    host, port, path, background, open_mail = read_command_line()
    message("Starting %s" % progname)
    if background:
        become_daemon()
    try:
        server = FakeServer((host, port), None, path, open_mail)
